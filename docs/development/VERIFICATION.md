...
P0-T03-knowledge-service-skeleton.md
```

Analysis-only tasks that do not change executable behavior, such as architecture investigation, do not require a user verification guide. They still require a development report.
...
2. **Prerequisites** — OS/runtime/version/configuration requirements.
...
5. Confirm Status is ready.
```

Do not write vague instructions such as:

```text
Verify that the plugin works correctly.
```
...
When a human verification run is performed, record its result in the task development report or a later validation report.
